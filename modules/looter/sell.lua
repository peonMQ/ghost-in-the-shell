...
local function sellItems()
  ...
  if not moveUtils.MoveToLoc(nearestMerchant.X(), nearestMerchant.Y(), nearestMerchant.Z(), 20, 12) then
      ...
      if inventoryItem() then
        if inventoryItem.Container() > 0 then
          for p=1,inventoryItem.Container() do
            logger.Info("Selling pack%d %d %s", i, p, inventoryItem.Item(p).Name())
