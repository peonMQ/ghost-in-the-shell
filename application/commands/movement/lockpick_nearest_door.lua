...
local function execute()
    ...
    local lockItem = nil
    if mq.TLO.FindItemCount("="..mechanizedLockpicks)() > 0 then
        lockItem = mechanizedLockpicks
    elseif mq.TLO.FindItemCount("="..lockpicks)() > 0 then
        lockItem = lockpicks
    end
    ...
    -- Lockpicks - 13010
    local invSlot = mq.TLO.FindItem("="..lockpicks).InvSlot()
    mq.cmd(string.format("/nomodkey /itemnotify %s leftmouseup", invSlot))
    mq.delay(2000, function() return mq.TLO.Cursor() == lockpicks end)
    if mq.TLO.Cursor() ~= lockpicks then
        logger.Error('|- Could not pick up <Lockpicks> on cursor.')
        mq.cmd("/beep")
    end
