...
local function canLootItem(item)
  ...
  if  mq.TLO.Me.FreeInventory() < 1 then
    if not item.Stackable() and mq.TLO.Me.FreeInventory() == 0 then
      return false
    end
  ...
  if not mq.TLO.Me.Casting.ID() then
    ...
    if mq.TLO.Spawn(closestCorpseID)() and mqUtils.EnsureTarget(closestCorpseID) then
      ...
      if target.Distance() and target.Distance() <= 20 and target.DistanceZ() < 40 then
        ...
        logger.Info("Corpse %s is %d|%d distance, skipping", target.Name() or 'nil', target.Distance() or -1, target.DistanceZ() or -1)
      end
    else
